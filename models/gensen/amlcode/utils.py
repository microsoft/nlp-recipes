...
class BufferedDataIterator(DataIterator):
    ...
    def __init__(
        ...
        self.f_src = [open(fname, 'r', encoding='utf-8') for fname in self.fname_src]
    ...
    def build_vocab(self):
        ...
        # If not, compute the vocab from scratch and store a cache.
            ...
            pickle.dump(
                ...
                open(os.path.join(self.save_dir, 'src_vocab.pkl'), 'wb', encoding='utf-8')
            )
        ...
        if os.path.exists(os.path.join(self.save_dir, 'trg_vocab.pkl')):
            ...
            pickle.dump(
                trg_vocab_dump,
                open(os.path.join(self.save_dir, 'trg_vocab.pkl'), 'wb', encoding='utf-8')
            )
...
def get_validation_minibatch(
    ...
    with torch.no_grad():
        ...
        sorted_src_lens = Variable(
            torch.LongTensor(sorted_src_lens)
        ).view(len(sorted_src_lens)).cuda()
    #print(input_lines_src.shape, input_lines_trg.shape, output_lines_trg.shape, sorted_src_lens.shape, sorted_src_lens)
    #print('*' * 20)
    #print(sorted_src_lines)
    return {
        'input_src': input_lines_src,
        'input_trg': input_lines_trg,
        'output_trg': output_lines_trg,
...
def compute_validation_loss(
    ...
    if lowercase:
        val_src = [line.strip().lower().split() for line in open(val_src, 'r', encoding='utf-8')]
        val_trg = [line.strip().lower().split() for line in open(val_trg, 'r', encoding='utf-8')]
    else:
        val_src = [line.strip().split() for line in open(val_src, 'r', encoding='utf-8')]
        val_trg = [line.strip().split() for line in open(val_trg, 'r', encoding='utf-8')]
