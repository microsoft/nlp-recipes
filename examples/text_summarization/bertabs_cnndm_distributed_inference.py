...
def shorten_dataset(dataset, rank=1,  top_n=-1, world_size=1):
    if world_size == 1:
        if top_n == -1:
            return dataset
...
def main_worker(local_rank, ngpus_per_node, summarizer, prediction_result, args):
    ...
    prediction = summarizer.predict(shorten_dataset(test_sum_dataset, rank=rank, top_n=TOP_N, world_size=world_size),
...
if __name__ == "__main__":
    with Manager() as manager:
        ...
        train_sum_dataset, test_sum_dataset = load_processed_cnndm_abs(args.data_dir)
        TOP_N = args.top_n
        summaries = [" ".join(summary).rstrip() for _, summary in shorten_dataset(test_sum_dataset, top_n=TOP_N)]
        ...
        rouge_score = get_rouge(predictions, summaries, RESULT_DIR)
        print(rouge_score)
