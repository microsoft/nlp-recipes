...
class Tokenizer:
    ...
    def preprocess_ner_tokens(
        ...
        Returns:
            tuple: A tuple containing the following two or three lists.
                1. input_ids_all: List of lists of integers. Each list contains
                    numerical values, i.e. token ids, corresponding tokens in
                    the input text data.
                2. input_mask_all: List of lists of integers, attention masks
                    of the input token id lists. 1 for input tokens and 0
                    for padded tokens, so that padded tokens are not
                    attended to.
                3. label_ids_all: List of lists of numerical labels,
        ...
        if label_available:
            return input_ids_all, input_mask_all, label_ids_all
        else:
            return input_ids_all, input_mask_all


def create_data_loader(input_ids, input_mask, label_ids=None,
                       sample_method='random', batch_size=32
                       ):
    """
    Create a dataloader for sampling and serving data batches.
    Args:
        input_ids (list): List of lists of integers. Each list contains
            numerical values, i.e. token ids, corresponding tokens in
            the input text data.
        input_mask (list): List of lists of integers, attention masks of
            the input token id lists. 1 for input tokens and 0 for padded
            tokens, so that padded tokens are not attended to.
        label_ids (list, optional): List of lists of numerical labels,
            each sublist contains token labels of a input
            sentence/paragraph. If provided, it's used to compute
            the evaluation loss. Default value is None.
    ...
    if label_ids:
        ...
        tensor_data = TensorDataset(
            input_ids_tensor, input_mask_tensor
        )
    ...
    if sample_method == "random":
        ...
        raise ValueError("Invalid sample_method value, accepted values are: "
